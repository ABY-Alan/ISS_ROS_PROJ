...
import test3_100_goals_door_world as door_world
...
TIMEOUT_PER_GOAL = door_world.TIMEOUT_PER_GOAL


def StartTest_DoorWorld_FiveModels(model_name: str, NUM_TRIALS: int = 100):
    ...
    base_file_name = f"Output_{get_output_tag(model_name)}_Door_World_Test.csv"

    rclpy.init()

    try:
        door_world.clear_all_world_models()
        delete_all_goals()

        output_dir = os.path.join(os.path.dirname(__file__), "Models", "Outputs")
        os.makedirs(output_dir, exist_ok=True)
        file_name = door_world.get_unique_filename(output_dir, base_file_name)
        recorder = DataRecorder(
            filename=os.path.join(output_dir, file_name),
            extra_fields=door_world.SCENE_EXTRA_FIELDS,
        )
        ...
        for i in range(1, NUM_TRIALS + 1):
            ...
            door_gap_width = door_world.sample_door_gap_width()
            wall_len = (door_world.DOOR_TOTAL_LENGTH - door_gap_width) / 2.0
            robot_x, robot_y, robot_yaw = door_world.sample_safe_robot_pose(door_gap_width)
            _, robot_clearance_to_wall = door_world._robot_clearance_metrics_at_pose(
                robot_x,
                robot_y,
                door_gap_width,
            )

            door_world.spawn_door_world_obstacle(
                sdf_template_path=sdf_template_path,
                door_gap_width=door_gap_width,
                model_name=door_world.DOOR_MODEL_NAME,
            )
            door_world.spawn_robot_entity(robot_x, robot_y, robot_yaw)

            random_x, random_y = door_world.sample_valid_goal(door_gap_width, robot_x, robot_y)
            ...
            reached = track_single_goal(
                ...
                collision_fail_distance_m=door_world.COLLISION_FAIL_DISTANCE_M,
                scene_data={
                    "trial_id": i,
                    ...
                    "world_center_x": door_world.WORLD_CENTER_X,
                    "world_center_y": door_world.WORLD_CENTER_Y,
                    "world_l1_radius": door_world.WORLD_L1_RADIUS,
                    "world_margin": door_world.WORLD_MARGIN,
                    "collision_fail_distance_m": door_world.COLLISION_FAIL_DISTANCE_M,
            ...
            print(
                f"耗时统计: "
                f"本轮={door_world._format_seconds(round_elapsed)} | "
                f"累计={door_world._format_seconds(total_elapsed)} | "
            )
...
if __name__ == "__main__":
    StartTest_DoorWorld_FiveModels("ppo_mixed_simple_fc_sft_epoch_020")
