...
def main():
	...
	try:
		...
		base_file_name = "Output_PPO_Ckpt_Door_World_Test_100.csv"
		...
		for i in range(1, NUM_TRIALS + 1):
			...
			robot_clearance_to_door, robot_clearance_to_wall = _robot_clearance_metrics_at_pose(
				robot_x,
				robot_y,
				door_gap_width,
			)
			...
			print(
				f"\n--- 第 {i}/{NUM_TRIALS} 轮: "
				f"机器人 ({robot_x:.2f}, {robot_y:.2f}, yaw={robot_yaw:.2f}), "
				f"目标 {goal_name} ({random_x:.2f}, {random_y:.2f}), "
				f"门洞宽度={door_gap_width:.2f}, 墙长={(DOOR_TOTAL_LENGTH - door_gap_width) / 2.0:.2f} ---"
			)
			...
			reached = track_single_goal(
				...
				scene_data={
					...
					"door_wall_length_left": round((DOOR_TOTAL_LENGTH - door_gap_width) / 2.0, 4),
					"door_wall_length_right": round((DOOR_TOTAL_LENGTH - door_gap_width) / 2.0, 4),
